...
| 2026-05-10 | **Qlik host strategy: `sense-demo.qlik.com` for dev, origin-inherited for QMC deploy**                                                                            | Dev runs against Qlik's public demo (anonymous, real apps, no infra setup). DEV/UAT/PRD all share the same code: when the zip is uploaded to QMC's content library, `location.host` resolves to whatever Qlik server hosts it — no URL config per environment. One env-var switch (`import.meta.env.PROD ? location.host : 'sense-demo.qlik.com'`) covers it. Implementation lands in Stage 7b alongside real `enigma.js`.                                                                                                                                           |
